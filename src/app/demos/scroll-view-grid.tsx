  ...
  const renderItem = useCallback(({ item }: { item: Item }) => (
    mode === 'grid' ? (
      <Surface elevation={0} style={[styles.gridItem, { backgroundColor: theme.colors.surfaceVariant }]}>
        <Text variant='labelMedium' numberOfLines={1} style={{ color: theme.colors.onSurfaceVariant }}>{item.key}</Text>
      </Surface>
    ) : (
  ...
  return (
    <View style={[styles.fill, { backgroundColor: theme.colors.background }]}>
      <ScrollViewProvider>
        ...
        <FlatList
          columnWrapperStyle={mode === 'grid' ? styles.row : undefined}
        ...
        <ScrollViewFooter style={styles.footer}>
          <Text variant='labelMedium' style={{ color: theme.colors.onSurfaceVariant }}>@rific/scroll-view · numColumns repro</Text>
        </ScrollViewFooter>
      </ScrollViewProvider>
    </View>
  )
